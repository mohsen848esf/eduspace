...
class RoomAdmin(admin.ModelAdmin):
    list_display = ('name', 'room_code', 'host', 'status', 'session', 'organization', 'meeting_type', 'max_participants', 'created_at')
    list_filter = ('status', 'meeting_type', 'organization')
...
class RecordingAdmin(admin.ModelAdmin):
    list_display = (
        'public_token', 'room', 'session', 'owner', 'status', 'quality',
        'duration_seconds', 'size_bytes', 'is_published', 'is_deleted',
        'started_at',
    )
    list_filter = ('status', 'quality', 'is_published', 'is_deleted', 'session')
