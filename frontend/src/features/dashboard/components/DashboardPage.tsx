...
export default function DashboardPage() {
  ...
  if (isTeacher && activeOrg) {
    ...
    const gradedSubmissions = allSubmissions.filter((sub) => sub.status === "graded" && sub.grade !== null);
    const avgStudentGrade = gradedSubmissions.length > 0
      ? Math.round(gradedSubmissions.reduce((acc, sub) => acc + parseFloat(sub.grade || "0"), 0) / gradedSubmissions.length)
      : 84;

    // 2. Class icons mappings
    const getSubjectIcon = (index: number) => {
      const icons = [
        // curly brackets
        <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-white">
    ...
    return (
      ...
      >
        <div className="flex flex-col gap-6 fade-in text-[var(--t1)]">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-[var(--t1)] tracking-tight">
              {isFarsi ? `خوش آمدید، استاد ${user?.full_name || user?.username}!` : `Welcome back, Professor ${user?.full_name || user?.username}!`}
            </h1>
            <p className="text-xs md:text-sm text-[var(--t3)] mt-1 font-medium">
              {isFarsi ? "خلاصه‌ای از وضعیت آموزشی و کلاس‌های امروز شما:" : "Here's your academic overview for today."}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-[var(--s2)] border border-[var(--b)] rounded-[24px] p-5 shadow-sm flex items-center justify-between gap-4">
              <div className="flex flex-col gap-1.5 min-w-0">
                <span className="text-[10px] font-bold text-[var(--t3)] uppercase tracking-wider">{isFarsi ? "ساعات تدریس" : "Taught Hours"}</span>
                <span className="text-xl md:text-2xl font-black text-[var(--t1)] leading-none">{totalTaughtHours} hrs</span>
                <span className="text-[10px] text-[var(--green)] font-bold flex items-center gap-0.5">
                  ▲ +12% {isFarsi ? "نسبت به ماه گذشته" : "vs last month"}
                </span>
              </div>
            ...
            <div className="bg-[var(--s2)] border border-[var(--b)] rounded-[24px] p-5 shadow-sm flex items-center justify-between gap-4">
              <div className="flex flex-col gap-1.5 min-w-0">
                <span className="text-[10px] font-bold text-[var(--t3)] uppercase tracking-wider">{isFarsi ? "کلاس‌های فعال" : "Active Classes"}</span>
                <span className="text-xl md:text-2xl font-black text-[var(--t1)] leading-none">{activeClassesCount}</span>
                <span className="text-[10px] text-[var(--t3)] font-semibold">{isFarsi ? "در ۲ دپارتمان آموزشی" : "Across 2 departments"}</span>
              </div>
              <div className="w-10 h-10 rounded-full bg-[var(--s3)] border border-[var(--b)] text-[var(--cyan)] flex items-center justify-center flex-shrink-0 text-lg">
                <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-[#0ea5e9]">
            ...
            <div className="bg-[var(--s2)] border border-[var(--b)] rounded-[24px] p-5 shadow-sm flex items-center justify-between gap-4">
              <div className="flex flex-col gap-1.5 min-w-0">
                <span className="text-[10px] font-bold text-[var(--t3)] uppercase tracking-wider">{isFarsi ? "بررسی‌های معوق" : "Pending Reviews"}</span>
                <div className="flex items-center gap-2">
                  <span className="text-xl md:text-2xl font-black text-[var(--t1)] leading-none">{pendingReviewsCount}</span>
                  <Link to="/academic/classes" className="px-2 py-0.5 rounded text-[8px] font-extrabold bg-[#6366f1] hover:bg-[#6366f1]/90 text-white no-underline tracking-wider uppercase transition-colors">
                    {isFarsi ? "تصحیح" : "Grade Now"}
                  </Link>
                </div>
                <span className="text-[10px] text-[var(--t3)] font-semibold">{isFarsi ? "پاسخ‌های در انتظار نمره" : "Submissions awaiting grade"}</span>
              </div>
              <div className="w-10 h-10 rounded-full bg-[var(--s3)] border border-[var(--b)] text-[var(--red)] flex items-center justify-center flex-shrink-0 text-lg animate-pulse">
                <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-[var(--red)]">
            ...
            <div className="bg-[var(--s2)] border border-[var(--b)] rounded-[24px] p-5 shadow-sm flex items-center justify-between gap-4">
              <div className="flex flex-col gap-1.5 min-w-0">
                <span className="text-[10px] font-bold text-[var(--t3)] uppercase tracking-wider">{isFarsi ? "میانگین نمرات" : "Avg Student Grade"}</span>
                <span className="text-xl md:text-2xl font-black text-[var(--t1)] leading-none">{avgStudentGrade}%</span>
                <span className="text-[10px] text-[var(--t3)] font-semibold">{isFarsi ? "ثابت در این ترم" : "Stable this term"}</span>
              </div>
              <div className="w-10 h-10 rounded-full bg-[var(--s3)] border border-[var(--b)] text-[var(--amber)] flex items-center justify-center flex-shrink-0 text-lg">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-[var(--s2)] border border-[var(--b)] rounded-[24px] p-5 shadow-sm lg:col-span-2 flex flex-col gap-4">
              <div className="flex justify-between items-center">
                ...
                <Link to="/academic/classes" className="text-xs font-semibold text-[var(--t3)] hover:text-[var(--brand)] no-underline flex items-center gap-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {myTaughtClasses.length === 0 ? (
                  <div className="col-span-full p-8 text-center text-xs text-[var(--t3)] bg-[var(--s3)] rounded-2xl border border-[var(--b)] border-dashed">
                    {isFarsi ? "شما کلاسی را در این ترم تدریس نمی‌کنید." : "You are not teaching any active classes currently."}
                  </div>
                ) : (
                  myTaughtClasses.slice(0, 2).map((c, idx) => {
                    ...
                    return (
                      <div key={c.id} className="bg-[var(--s3)] border border-[var(--b)] rounded-2xl p-5 flex flex-col justify-between gap-4 group hover:border-[var(--brand)]/30 transition-all shadow-sm">
                        <div className="flex flex-col gap-3">
                          <div className="flex justify-between items-center">
                            <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center shadow-md", classThemeBg)}>
                              {classThemeIcon}
                            </div>
                            <span className={cn(
                              "text-[8px] font-black px-2 py-0.5 rounded tracking-wider uppercase",
                              isFirst ? "bg-emerald-500/10 text-emerald-500 animate-pulse" : "bg-[var(--s2)] text-[var(--t3)]"
                            )}>
                              {isFirst ? (isFarsi ? "کلاس تا ۱۰ دقیقه دیگر" : "Live in 10m") : "14:00 PM"}
                            </span>
                          </div>
                          <div>
                            <h3 className="text-sm font-extrabold text-[var(--t1)]">{c.name}</h3>
                            <p className="text-[10px] text-[var(--t3)] font-semibold mt-0.5 truncate">{c.course_title || (isFarsi ? "عنوان درس" : "Subject details")}</p>
                          </div>
                          <span className="text-[10px] text-[var(--t2)] font-semibold flex items-center gap-1 mt-1 select-none">
                            👥 {enrollmentCount} {isFarsi ? "دانشجو ثبت نام شده" : "Students enrolled"}
                          </span>
                        </div>
                        <div className="border-t border-[var(--b)] pt-3.5 mt-1">
                          {isFirst ? (
                            <Link
                              ...
                              className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-[#6366f1] hover:brightness-110 text-white font-bold text-xs cursor-pointer border-none shadow-md transition-all active:scale-[0.98] no-underline"
                            >
                              <span>🚪</span>
            ...
            <div className="bg-[var(--s2)] border border-[var(--b)] rounded-[24px] p-5 shadow-sm flex flex-col gap-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[8px] font-black px-2 py-0.5 rounded bg-red-500/10 text-red-500 uppercase tracking-wider animate-pulse">
                  2 NEW
                </span>
              </div>
              ...
              <div className="flex flex-col gap-2.5">
                ...
                <div className="bg-[var(--s3)] border border-[var(--b)] rounded-2xl p-3.5 flex items-start gap-3 shadow-inner">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-[11px] font-bold text-[var(--t1)] truncate">Alex J. - CS102 Quiz 3</span>
                    <span className="text-[9px] text-[var(--red)] font-extrabold mt-0.5">{isFarsi ? "خروج مکرر از صفحه آزمون (۴ بار)" : "High Tab Focus Loss detected (4 times)"}</span>
                    <span className="text-[8px] text-[var(--t3)] font-semibold mt-1">10 mins ago</span>
                  </div>
                </div>
                ...
                <div className="bg-[var(--s3)] border border-[var(--b)] rounded-2xl p-3.5 flex items-start gap-3 shadow-inner">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-[11px] font-bold text-[var(--t1)] truncate">Sarah M. - CS102 Quiz 3</span>
                    <span className="text-[9px] text-[var(--red)] font-extrabold mt-0.5">{isFarsi ? "چهره بر روی دوربین شناسایی نشد" : "Face not detected on camera"}</span>
                    <span className="text-[8px] text-[var(--t3)] font-semibold mt-1">25 mins ago</span>
                  </div>
                </div>
                ...
                <div className="bg-[var(--s3)] border border-[var(--b)] rounded-2xl p-3.5 flex items-start gap-3 shadow-inner opacity-60">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-[11px] font-bold text-[var(--t1)] truncate">David L. - CS102 Quiz 3</span>
                    <span className="text-[9px] text-[var(--t3)] font-semibold mt-0.5">{isFarsi ? "قطع شدن ورودی میکروفون" : "Audio input disconnected"}</span>
                    <span className="text-[8px] text-[var(--t3)] font-semibold mt-1">1 hr ago • Resolved</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-[var(--s2)] border border-[var(--b)] rounded-[24px] p-6 shadow-sm lg:col-span-2 flex flex-col justify-between gap-5">
              <div>
                <h2 className="text-xs font-bold text-[var(--t2)] uppercase tracking-wider flex items-center gap-2">
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-[var(--t3)]">
              ...
              <div className="border-t border-[var(--b)] pt-3.5 mt-1">
                <button
                  onClick={() => toast.success(isFarsi ? "دانلود گزارش آغاز شد..." : "Downloading detailed attendance report...")}
            ...
            <div className="bg-[var(--s2)] border border-[var(--b)] rounded-[24px] p-5 shadow-sm flex flex-col gap-4">
              ...
              {/* Timeline Track */}
              <div className="flex flex-col gap-4 relative border-s-2 border-[var(--b)] ms-2 ps-4.5 py-1">
                {/* Timeline item 1 */}
                <div className="relative">
                  <span className="absolute -start-[23px] top-1 w-2.5 h-2.5 rounded-full bg-[#6366f1] ring-4 ring-[var(--s2)] shadow-sm" />
                  <span className="text-[9px] font-black text-[#6366f1] font-mono block">10:00 AM - 11:30 AM</span>
                  <div className="bg-[var(--s3)] border border-[var(--b)] rounded-xl p-3.5 mt-2 flex flex-col gap-3 shadow-inner">
                    <div className="flex flex-col">
                      <span className="text-[11px] font-bold text-[var(--t1)]">CS102: Data Structures</span>
                      <span className="text-[9px] text-[var(--t3)] font-semibold mt-0.5">📂 Lecture Hall A (Virtual)</span>
                    </div>
                    <button
                      onClick={() =>
                        createRoom({
                      ...
                      className="w-full flex items-center justify-center gap-1 py-1.5 rounded-lg bg-[#6366f1] hover:brightness-110 text-white font-bold text-[10px] cursor-pointer border-none transition-all active:scale-[0.98]"
                    >
                ...
                <div className="relative mt-2">
                  ...
                  <span className="text-[9px] font-black text-[var(--t3)] font-mono block">14:00 PM - 15:30 PM</span>
                  <div className="bg-[var(--s3)] border border-[var(--b)] rounded-xl p-3.5 mt-2 flex flex-col shadow-inner">
                    <span className="text-[11px] font-bold text-[var(--t1)]">CS305: Computer Arch.</span>
                    <span className="text-[9px] text-[var(--t3)] font-semibold mt-0.5">🔬 Lab Session</span>
                  </div>
                </div>
                ...
                <div className="relative mt-2">
                  <span className="absolute -start-[23px] top-1 w-2.5 h-2.5 rounded-full bg-[var(--b)] ring-4 ring-[var(--s2)]" />
                  <span className="text-[9px] font-black text-[var(--t3)] font-mono block">16:00 PM - 17:00 PM</span>
                  <div className="bg-[var(--s3)] border border-[var(--b)] rounded-xl p-3.5 mt-2 flex flex-col shadow-inner">
                    <span className="text-[11px] font-bold text-[var(--t1)]">{isFarsi ? "ساعات پاسخگویی استاد" : "Office Hours"}</span>
                    <span className="text-[9px] text-[var(--t3)] font-semibold mt-0.5">👥 Open Booking</span>
                  </div>
                </div>
              </div>
            </div>
